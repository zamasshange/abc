...
export function DrawingShell({ children, onBack, showCanvas = true, header }: {
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-white">
      <LeftToolbar isEraser={isEraser} onEraserToggle={() => setIsEraser((p) => !p)} onPenSelect={() => setIsEraser(false)}
        onClear={() => setClearToken((t) => t + 1)} onBack={() => setShowExit(true)} />
      <div className="relative flex min-h-0 min-w-0 flex-1 flex-col bg-white">
        {header}
        <div className="relative min-h-0 flex-1">
          {children}
          {showCanvas && <DrawingCanvas strokeColor={strokeColor} strokeWidth={14} isEraser={isEraser} clearToken={clearToken} />}
        </div>
      </div>
      <RightToolbar selectedColor={strokeColor} isEraser={isEraser}
        onColorSelect={(c) => { setStrokeColor(c); setIsEraser(false); }} onDone={onBack} />
